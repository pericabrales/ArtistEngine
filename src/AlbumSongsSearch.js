...
function AlbumSongsSearch({album}){
    ...
    console.log("album upon entrance: ", album);
        ...
        async function fetchAlbumSongs(){
            ...
            try{
               ...
               jsResponse = await fetch(req).then(res => {
                ...
                //album stuff
                const albumReq = new Request(`https://api.spotify.com/v1/albums/${album}`, {
                    headers: new Headers({
                        'Authorization': token[0].token_type + ' ' + token[0].access_token
                    })
                });

               jsResponseNew = await fetch(albumReq).then(res => {
                    if(res.statusText === "Unauthorized"){
                        window.location.href = './';
                    }
                    return res.json();
                }).then(res => {
                    console.log("album res items: ", res);
                    return res;
                }).catch(e => {
                    //dispatch(albumSongsError(e));
                });
                ...
                console.log("full api call album: ", jsResponseNew);
            ...
            if(!ignore){
                ...
                setRepos(jsResponse || []);
                setAlbumInfo(jsResponseNew || []);
                //set back to false because we have our stuff
                setLoadingState(false);
                console.log("==repos: ", jsResponse);
                console.log("==albumInfo: ", jsResponseNew);
                console.log("==albumInfo name: ", jsResponseNew.name);
            } 
        }
    ...
    return(
        <div id="album-songs-search">
            <h3 id="page-title-album-songs">Song Gallery</h3>
            {repos.map(i => 
